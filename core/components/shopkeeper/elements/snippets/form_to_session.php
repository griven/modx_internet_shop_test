...
if( !empty( $_POST[$submitVar] ) ){
    ...
    $modx->registry->form_to_session->send(
        ...
        array( 'kill' => true, 'expires' => time() + 1440 )
    );
            ...
            foreach( $form_ids_arr as $v ){
                ...
                if( is_numeric( $f_id ) ){
                    ...
                    if( $form ){
                        ...
                        foreach( $form_data as $s_k => $s_val ){
                            
                            if( strpos( $s_k, 'field' . $f_id . '_' ) !== false ){
                                $form_fields[$s_k] = $s_val;
                            }
                        ...
                        foreach($form_fields_data as $k => $field){
                            ...
                            $field_name = 'field' . $f_id . '_' . ($k+1);
                            
                            if( !empty( $form_fields[$field_name] ) ){
                                
                                $value = is_array($form_fields[$field_name]) ? implode(', ',$form_fields[$field_name]) : $form_fields[$field_name];
